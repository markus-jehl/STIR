...
FBP2DReconstruction::set_up(shared_ptr<FBP2DReconstruction::TargetT> const& target_data_sptr)
{
  ...
  if (fc_ramp <= 0 || fc_ramp > .5000000001)
    error(fmt::format("Cut-off frequency has to be between 0 and .5 but is {}", fc_ramp));

  if (alpha_ramp <= 0 || alpha_ramp > 1.000000001)
    error(fmt::format("Alpha parameter for ramp has to be between 0 and 1 but is {}" , alpha_ramp));

  if (pad_in_s < 0 || pad_in_s > 2)
    error(fmt::format("padding factor has to be between 0 and 2 but is {}" , pad_in_s));
  ...
  if (num_segments_to_combine >= 0 && num_segments_to_combine % 2 == 0)
    error(fmt::format("num_segments_to_combine has to be odd (or -1), but is {}", num_segments_to_combine));
...
FBP2DReconstruction::FBP2DReconstruction(const std::string& parameter_filename)
{
  ...
  info(fmt::format("{}", parameter_info()));
}
...
#  pragma omp critical(FBP2D_get_viewgrams)
        ...
        info(fmt::format("Processing view {} of segment {}", vs.view_num(), vs.segment_num()), 2);
