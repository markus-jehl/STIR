...
void multiply_crystal_factors(ProjData& proj_data, const Array<2,float>& efficiencies, const float global_factor)
{
	if (proj_data.get_proj_data_info_sptr()->get_scanner_ptr()->get_scanner_geometry() == "BlocksOnCylindrical")
	{
		...
		const int num_detectors_per_ring = scanner_sptr->get_num_detectors_per_ring();
		unique_ptr<ProjDataInfo> uncompressed_proj_data_info_uptr(ProjDataInfo::construct_proj_data_info(
				scanner_sptr, /*span=*/1, max_ring_diff, /*num_views=*/num_detectors_per_ring / 2,
				scanner_sptr->get_max_num_non_arccorrected_bins(), /*arccorrection=*/false));
		const ProjDataInfoBlocksOnCylindricalNoArcCorr* const uncompressed_proj_data_info_ptr
				= dynamic_cast<const ProjDataInfoBlocksOnCylindricalNoArcCorr* const>(uncompressed_proj_data_info_uptr.get());

		// some fields need to be initialised, because they're used (in get_m() for BlocksOnCylindrical) before being set
		Bin bin;
		bin.tangential_pos_num() = 0;
		bin.view_num() = 0;
		Bin uncompressed_bin;
		uncompressed_bin.tangential_pos_num() = 0;
		uncompressed_bin.view_num() = 0;
						...
						for (int view_num = proj_data.get_min_view_num(); view_num <= proj_data.get_max_view_num(); ++view_num)
						{
									 ...
									 ++tangential_pos_num)
							{
										 ...
										 ++parallel_uncompressed_bin.view_num())
								{
									...
									uncompressed_proj_data_info_ptr->get_det_pair_for_bin(a, ra, b, rb, parallel_uncompressed_bin);
...
#  endif
									...
									sinogram[parallel_bin.view_num()][parallel_bin.tangential_pos_num()]
											+= result * global_factor;
	...
	else
	{
					...
					++bin.segment_num())
		{
						...
						bin.axial_pos_num() <= proj_data.get_max_axial_pos_num(bin.segment_num()); ++bin.axial_pos_num())
			{
				...
				// now loop over uncompressed detector-pairs
				for (uncompressed_bin.segment_num() = in_min_segment_num;
							uncompressed_bin.segment_num() <= in_max_segment_num; ++uncompressed_bin.segment_num())
				{
					for (uncompressed_bin.axial_pos_num()
								= uncompressed_proj_data_info_ptr->get_min_axial_pos_num(uncompressed_bin.segment_num());
								uncompressed_bin.axial_pos_num()
...
#  endif
#endif
						for (int view_num = proj_data.get_min_view_num(); view_num <= proj_data.get_max_view_num();
									++view_num)
						{
							for (int tangential_pos_num = proj_data_info_ptr->get_min_tangential_pos_num();
										tangential_pos_num <= proj_data_info_ptr->get_max_tangential_pos_num();
										++tangential_pos_num)
							{
								...
								for (parallel_uncompressed_bin.view_num() = parallel_bin.view_num() * mashing_factor;
											parallel_uncompressed_bin.view_num()
											< (parallel_bin.view_num() + 1) * mashing_factor;
											++parallel_uncompressed_bin.view_num())
								{
